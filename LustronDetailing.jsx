...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sv-step-num.active { background: #2563eb; color: #fff; }
  .sv-step-num.inactive { background: rgba(37,99,235,0.15); color: rgba(226,232,240,0.4); }
  .sv-step-text {
    font-size: 0.72rem;
    font-weight: 700;
  ...
  @media (max-width: 640px) {
    .sv-vehicle-grid { grid-template-columns: repeat(3, 1fr); }
  }
  .sv-vehicle-btn {
    display: flex;
    flex-direction: column;
  ...
  .sv-vehicle-emoji { font-size: 2rem; line-height: 1; }
  .sv-vehicle-name {
    font-size: 0.65rem;
    font-weight: 700;
    letter-spacing: 0.1em;
  ...
  .sv-vehicle-btn.selected .sv-vehicle-name { color: #60a5fa; }
  ...
  .sv-type-btn.selected .sv-type-name { color: #93c5fd; }
  .sv-type-desc {
    font-size: 0.75rem;
    font-weight: 300;
  ...
  .sv-pkg-body { padding: 1.5rem; flex: 1; display: flex; flex-direction: column; gap: 1rem; }
  .sv-pkg-badge {
  ...
  .sv-feature-check { flex-shrink: 0; margin-top: 1px; font-size: 0.75rem; }
  .sv-book-btn {
    width: 100%;
    padding: 12px;
    border-radius: 8px;
  ...
  .sv-book-btn:hover { opacity: 0.9; transform: scale(1.02); }
  ...
  .sv-modal {
    ...
    max-height: 90vh;
    overflow-y: auto;
    animation: ld-scalein 0.25s cubic-bezier(0.34,1.56,0.64,1);
  }
  .sv-modal-title {
    font-family: 'Bebas Neue', cursive;
    font-size: 2rem;
  ...
  .sv-summary-val.blue { color: #3b82f6; }
  ...
  .sv-err { color: #f87171; font-weight: 600; }
  .sv-input {
    width: 100%;
  ...
  .sv-select option { background: #081629; color: #e2e8f0; }
  .sv-modal-footer {
    font-size: 0.72rem;
    font-weight: 300;
    color: rgba(226,232,240,0.35);
  ...
  .sv-success { text-align: center; padding: 2rem 1rem; }
        ...
        {sent ? (
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem" }}>
              ...
              <button onClick={onClose} style={{ background: "rgba(37,99,235,0.12)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: "8px", color: "rgba(226,232,240,0.65)", cursor: "pointer", fontSize: "1.2rem", lineHeight: 1, padding: "3px 10px", fontFamily: "inherit" }}>×</button>
            </div>
...
function ServicesPage({ onNavigate }) {
  ...
  const tierColor = { basic: "#475569", popular: "#2563eb", premium: "#1d4ed8", elite: "#1e3a8a" };

  return (
    <div className="sv-page">
      <div className="sv-header">
        <div className="ld-badge" style={{ marginBottom: "1rem", display: "inline-flex" }}>
          <div className="ld-badge-dot" />
          Mobile Detailing
        </div>
        <h1 className="ld-h2" style={{ marginBottom: "0.5rem" }}>Our Services</h1>
        <p style={{ fontSize: "1rem", fontWeight: 300, color: "rgba(226,232,240,0.55)", maxWidth: "460px", margin: "0 auto", lineHeight: 1.7 }}>
      ...
      <div className="sv-container">
        ...
        <div className="sv-vehicle-grid" style={{ marginBottom: "2.5rem" }}>
          {VEHICLES.map(v => (
            <button key={v.id} className={`sv-vehicle-btn${vehicle === v.id ? " selected" : ""}`} onClick={() => pickVehicle(v.id)}>
              <span className="sv-vehicle-emoji">{v.emoji}</span>
        ...
        {/* STEP 2 */}
        <div id="sv-step2" style={{ overflow: "hidden", maxHeight: vehicle ? "400px" : "0", transition: "max-height 0.5s ease", opacity: vehicle ? 1 : 0 }}>
          <div className="sv-step-label">
            <div className={`sv-step-num ${step >= 2 ? "active" : "inactive"}`}>2</div>
            <span className="sv-step-text">Choose Service Type{veh && <em>— {veh.name}</em>}</span>
          </div>
          <div className="sv-type-row" style={{ marginBottom: "2.5rem" }}>
            <button className={`sv-type-btn${svcType === "interior" ? " selected" : ""}`} onClick={() => pickType("interior")}>
            ...
            <button className={`sv-type-btn${svcType === "exterior" ? " selected" : ""}`} onClick={() => pickType("exterior")}>
              <span className="sv-type-icon">✨</span>
        ...
        {/* STEP 3 */}
        <div id="sv-step3" style={{ overflow: "hidden", maxHeight: svcType ? "3000px" : "0", transition: "max-height 0.6s ease", opacity: svcType ? 1 : 0 }}>
          <div className="sv-step-label">
          ...
          <div className="sv-pkg-grid">
            {pkgs.map((pkg, i) => {
              const col = tierColor[pkg.tier];
              const tc = TIER_CFG[pkg.tier];
              return (
                <div key={`${vehicle}-${svcType}-${pkg.name}`} className="sv-pkg-card anim-su"
                  style={{ animationDelay: `${i * 0.08}s`, borderColor: `${col}33` }}>
                  <div className="sv-pkg-top" style={{ background: `linear-gradient(90deg, transparent, ${col}, transparent)` }} />
                  <div className="sv-pkg-body">
                    {pkg.tier !== "basic" && (
                      <span className="sv-pkg-badge" style={{ background: `${col}22`, border: `1px solid ${col}55`, color: col }}>
                        {tc.label}
                      </span>
                    )}
                    <div className="sv-pkg-name">{pkg.name}</div>
                    <div className="sv-pkg-price" style={{ color: col }}>${pkg.price}</div>
                    <ul className="sv-features">
                      {pkg.features.map((f, fi) => (
                        <li key={fi} className="sv-feature">
                          <span className="sv-feature-check" style={{ color: col }}>✓</span>
                          {f}
                        </li>
                      ))}
                    </ul>
                    <button
                      ...
                      style={{ background: `linear-gradient(135deg, ${col} 0%, ${col}cc 100%)` }}
                      onClick={() => setBooking({ vehicle: veh?.name || "", service: svcType === "interior" ? "Interior Detailing" : "Exterior Detailing", package: pkg.name, price: pkg.price })}
                    >
                      Book Now →
                    </button>
                  </div>
        ...
        {/* Trust bar */}
        <div style={{ marginTop: "4rem", paddingTop: "2rem", borderTop: "1px solid rgba(37,99,235,0.15)", display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "2rem" }}>
          {[
            ...
            <div key={text} style={{ display: "flex", alignItems: "center", gap: "8px", color: "rgba(226,232,240,0.5)" }}>
              <span style={{ color: "#60a5fa" }}>{icon}</span>
              <span style={{ fontSize: "0.8rem", fontWeight: 500 }}>{text}</span>
            </div>
          ))}
        </div>
      </div>
